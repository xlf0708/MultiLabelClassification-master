...
from lib.dataset import MLDataset, SCWTJSONDataset
...
class Trainer(object):
    def __init__(self, args):
        ...
        #     )
        # ])
        # train_dataset = MLDataset(args.train_path)
        self.train_loader = DataLoader(dataset=SCWTJSONDataset(args.train_path, 'train'), batch_size=256)
        ...
        #     )
        # ])
        # val_dataset = MLDataset(args.val_path, args.label_path)
        ...
        #     pin_memory=True
        # )
        self.train_loader = DataLoader(dataset=SCWTJSONDataset(args.train_path, 'train'), batch_size=32)
        self.val_loader = DataLoader(dataset=SCWTJSONDataset(args.train_path, 'train'), batch_size=32)
        # self.model = model_factory[args.model](args, args.num_classes)
        self.model = ResNet(blocks=4,num_classes=4)
        self.model.cuda()
    ...
    def validation(self, epoch):
        ...
        with torch.no_grad():
            for _, batch in enumerate(self.val_loader):
                ...
                loss = loss.cpu().numpy()
                acc += (pred_y.argmax(1) == y.argmax(1)).type(torch.float).sum().item()
       ...
       # _, mAP = self.voc12_mAP.compute()
        ...
        print("Validation [epoch {}] acc:{:.4f} loss: {:.4f}".format(epoch,acc,  mLoss))
        return acc
